...
When evaluating the CFE, we only consider the bound fraction of star formation and omit the `cruel cradle effect', i.e.~the tidal disruption of star-forming overdensities by dense substructure in the ISM. This choice is made, because the clouds on the CMZ dust ridge undergo coherent motion on a thin, circumnuclear stream, which strongly limits the encounter rate relative to galactic discs, in which encounters with GMCs may occur in three-dimensional rather than one-dimensional space.
