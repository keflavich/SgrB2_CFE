...
\begin{abstract}
    ...
    of the cluster formation efficiency (CFE) on the local gas density, indicating
    that stars preferentially form in star clusters when the star formation
    rate is higher.  Previously, it has not been possible to test this relation
    at very early times and in clusters with individual stars resolved because
    of the small variation in densities in the cluster-forming regions in the
    local group.  However, in our Galaxy's central molecular zone (CMZ), there
    are clouds with densities much higher than anywhere else in the local
    group.  We report a measurement of the CFE, the fraction of stars forming
    in clusters, in the highest-density region in the Galaxy, Sgr B2.  We find
    that over a third of the stars (37-43\%) in Sgr B2 are forming in bound
    clusters. This value is consistent with the predictions of the
    \citet{Kruijssen2012a} models and are inconsistent with a constant CFE in
    the Galaxy.
\end{abstract}
...
great distances \citep[e.g.,][]{Brodie2006a,Kruijssen2018a} and large coeval
and co-located samples of stars in the local universe.  The prevalence of these
clusters varies substantially with environment: the \emph{cluster formation
efficiency} $\Gamma$ is not constant \citep{Adamo2015a,Johnson2016a,Messa2018a}.
...
density, while in less ordered environments, it is the volume density} and
other global gas parameters.  While this theory reasonably explains
observations spanning many galaxies, it has not yet been directly tested in a
high-density environment where both the unclustered and clustered stars are
detected.  In this Letter, we perform such a test in the Sgr B2 cloud, a
high-density region in the Galactic center in which both stars and bound
clusters are presently forming.
...
The stellar mass we adopt for the cluster, $M_M=1.5\ee{4}$ \msun, is therefore
a lower limit.  Adopting a higher value for $M_M$, and correspondingly decreasing
...
cases, $\sigma_{1D}$ is  lower than the escape velocity
...
\citet{Lada2003a} reported a CFE$=7^{+7}_{-3}\%$ for the local neighborhood, a
measurement $4\sigma$ below our lower value.  Our observation therefore confirms
...
global Galactic-scale and local physical gas conditions.  Using observed parameters
...
Sgr B2, $\Gamma=37-43\%$.  Since this CFE is higher than that in the local
neighborhood, this measurement is evidence for a non-uniform CFE, in particular
one that depends on local gas conditions.  The CFE model of \citet{Kruijssen2012a},
in which higher average gas densities yield higher CFEs, successfully predicts
the observed value to within a factor of 2.
